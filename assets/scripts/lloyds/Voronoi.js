class VoronoiDiagram {
	...
	circle_event(e) {
		...
		if (!this.point_outside(e.vertex)) {
			this.voronoi_vertex.push(e.vertex); // Only add the vertex if inside canvas
			arc.focus.addVertex = arc.left.focus.addVertex = arc.right.focus.addVertex = e.vertex;
		...
		while (r.right) {
			...
			if (
				...
				if (e.m == 0) {
					...
					e.arc.left.addVertex = e.arc.right.addVertex = e.end;
		...
		for (let i = 0; i < this.edges.length; i++) {
			...
			switch (option) {
				case 3: // Both endpoints outside the canvas
					this.edges[i] = null;
					// ERROR ------>> Misses the case where a site is close to an angle and the edge cuts across the angle
					break;
	...
	edge_end(e, y_lim) {
		...
		e.arc.left.addVertex = e.arc.right.addVertex = p;
		return p;
	}
...
class Point {
	...
	set addVertex(v){
		if(this.vertices == undefined) this.vertices = [v];
